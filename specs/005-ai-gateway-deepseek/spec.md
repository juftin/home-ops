...
As a homelab operator, I want to send OpenAI-format API requests to `ai.<domain>` and have them
proxied through the Envoy AI Gateway to the appropriate provider based on model name. Initially all
models route to DeepSeek; future providers (Anthropic, OpenAI) will be added with model-specific
routing rules.
...
2. **Given** the AI Gateway is deployed, **When** a request includes any model name, **Then** the
   request is routed to the DeepSeek backend without requiring per-model route rules (catch-all).
...
- **FR-003**: Two Backend resources MUST be configured — one for each DeepSeek API base URL:
  `api.deepseek.com:443` and `api.deepseek.com:443` (Anthropic endpoint at `/anthropic` path).
- **FR-004**: Two AIServiceBackends MUST be configured:
  - `deepseek-openai` with OpenAI schema → Backend `deepseek`
  - `deepseek-anthropic` with Anthropic schema → Backend `deepseek-anthropic`
- **FR-005**: Two BackendSecurityPolicies of type APIKey MUST be configured (one per
  AIServiceBackend), both referencing the same `deepseek-api-key` Secret.
...
- **Backend/deepseek**: Points to `api.deepseek.com:443` — the OpenAI-compatible endpoint.
- **Backend/deepseek-anthropic**: Points to `api.deepseek.com:443` at path `/anthropic` — DeepSeek's
  Anthropic-compatible endpoint.
- **AIServiceBackend/deepseek-openai**: Wraps Backend `deepseek` with OpenAI schema.
- **AIServiceBackend/deepseek-anthropic**: Wraps Backend `deepseek-anthropic` with Anthropic schema.
- **BackendSecurityPolicy/deepseek-openai** and **deepseek-anthropic**: Both inject the same API key
  from `deepseek-api-key` Secret into their respective backends.
- **AIGatewayRoute/deepseek**: Catch-all route attaching both DeepSeek AIServiceBackends. The AI
  Gateway detects the incoming request format and routes to the matching schema.
...
- **SC-001**: An OpenAI-format chat completion request sent to
  `https://ai.<domain>/v1/chat/completions` with any DeepSeek model name returns a valid DeepSeek
  response.
...
- Q: API formats? → A: DeepSeek publishes two API endpoints — `api.deepseek.com` (OpenAI format)
  and `api.deepseek.com/anthropic` (Anthropic format). Two AIServiceBackends are needed, each with
  the matching schema. Both use the same API key.
